...
class ParcelViewModel: ObservableObject {
    @Published var parcels: [ParcelModel] = []
    
    // Read
    func fetch() {
        var request = URLRequest(url: URL(string: "https://vceggkauetnfughruuzh.supabase.co/rest/v1/Parcel?select=*")!,timeoutInterval: Double.infinity)
        ...
        let task = URLSession.shared.dataTask(with: request) { data, response, error in
            ...
            do {
                let decodedData = try JSONDecoder().decode([ParcelModel].self, from: data)
                DispatchQueue.main.async {
                    self.parcels = decodedData
                }
            } catch {
